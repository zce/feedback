 ...
 * }
 */
const fs = require(process.versions.electron ? 'original-fs' : 'fs')
const os = require('os')
...
class Updater extends EventEmitter {
  ...
  _download (promise) {
    return promise.then(task => {
      if (task.available) {
        return this._fetchFile(task.remote.url, task.name)
  ...
  _allCompleted (tasks) {
    fs.writeFile(path.resolve(this.cacheDirectory, 'manifest.json'), JSON.stringify(this.manifest), 'utf8', error => {
